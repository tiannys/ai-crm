...
function getOpenAIClient(): OpenAI | null {
  ...
  if (!apiKey) {
    log.warn('OPENAI_API_KEY not set in .env — using fallback');
    return null;
  }

  // Support Gemini via OpenAI-compatible endpoint
  const baseURL = process.env.OPENAI_BASE_URL || undefined;
  if (baseURL) {
    log.info({ baseURL }, 'Using custom AI endpoint (Gemini)');
  }
...
Return JSON with this exact structure:
{
  ...
  const result = await callOpenAI(systemPrompt, context);
